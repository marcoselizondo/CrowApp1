...
/** Video with punch-in reveal + decaying camera shake. */
const DynClip: React.FC<{ src: string; shake?: number }> = ({ src, shake = 16 }) => {
  ...
  const amp = interpolate(frame, [0, 16], [shake, 0], { extrapolateRight: "clamp" });
...
const Flash: React.FC = () => {
  const frame = useCurrentFrame();
  const o = interpolate(frame, [0, 3, 7], [0.85, 0.3, 0], { extrapolateRight: "clamp" });
...
const Scrim: React.FC = () => (
  <AbsoluteFill
    style={{
      background:
        "linear-gradient(to top, rgba(0,0,0,0.62) 0%, rgba(0,0,0,0.2) 26%, rgba(0,0,0,0) 44%), linear-gradient(to bottom, rgba(0,0,0,0.42) 0%, rgba(0,0,0,0) 20%)",
    }}
  />
);

const PriceStamp: React.FC<{ at: number }> = ({ at }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const s = spring({
    frame: frame - at,
  ...
  const op = interpolate(frame - at, [0, 4], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  return (
    <div
      style={{
        position: "absolute",
...
const SceneBlock: React.FC<{ scene: Scene; sceneDuration: number }> = ({ scene, sceneDuration }) => {
  let offset = 0;
  return (
    <AbsoluteFill>
      {scene.clips.map((c, i) => {
      ...
      <Sequence durationInFrames={sceneDuration}>
        {scene.idx !== null ? <Counter idx={scene.idx} /> : null}
        {scene.idx !== null ? <PriceStamp at={10} /> : null}
...
const HookTitle: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", padding: "0 70px" }}>
      ...
      >
        <div style={{ fontSize: 168, color: COLORS.green }}>Día 5</div>
  ...
  return (
    ...
    >
      <div style={{ transform: `translateY(${interpolate(pop, [0, 1], [30, 0])}px)`, opacity: pop, marginBottom: 40 }}>
        <div style={{ fontSize: 62, fontWeight: 800, color: COLORS.ink, lineHeight: 1.12 }}>
      ...
      <div style={{ opacity: cta, transform: `scale(${interpolate(cta, [0, 1], [0.9, 1])})`, display: "flex", flexDirection: "column", alignItems: "center" }}>
        <div style={{ background: "#fff", padding: 26, borderRadius: 48, boxShadow: "0 20px 50px rgba(20,38,27,0.28)", marginBottom: 30 }}>
          <DonnitLogo size={140} />
        </div>
        <div style={{ fontSize: 80, fontWeight: 800, color: COLORS.ink, letterSpacing: -1 }}>Descarga Donnit</div>
        <div style={{ fontSize: 38, fontWeight: 700, color: COLORS.ink, opacity: 0.9, marginTop: 16 }}>Ve lo que regalan en tu barrio 👆 link en bio</div>
      </div>
    </AbsoluteFill>
  );
};

export const Reel16: React.FC = () => {
